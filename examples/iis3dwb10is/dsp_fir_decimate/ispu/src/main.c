...
static struct ispu_fir_decimate_instance_f32 fir_decimate_x, fir_decimate_y, fir_decimate_z;
...
void __attribute__ ((signal)) algo_00_init(void)
{
	...
	ispu_fir_decimate_init_f32(&fir_decimate_x, FIR_TAPS, DEC, coeffs, state_x, BLOCK_SIZE);
	ispu_fir_decimate_init_f32(&fir_decimate_y, FIR_TAPS, DEC, coeffs, state_y, BLOCK_SIZE);
	ispu_fir_decimate_init_f32(&fir_decimate_z, FIR_TAPS, DEC, coeffs, state_z, BLOCK_SIZE);
}
...
void __attribute__ ((signal)) algo_00(void)
{
	if (full) {
		...
		ispu_fir_decimate_f32(&fir_decimate_x, (float *)acc_x[proc], out_x[proc], BLOCK_SIZE);
		ispu_fir_decimate_f32(&fir_decimate_y, (float *)acc_y[proc], out_y[proc], BLOCK_SIZE);
		ispu_fir_decimate_f32(&fir_decimate_z, (float *)acc_z[proc], out_z[proc], BLOCK_SIZE);
	}
