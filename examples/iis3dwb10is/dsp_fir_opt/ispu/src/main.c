...
static struct ispu_fir_instance_f32_opt fir_x, fir_y, fir_z;
...
void __attribute__ ((signal)) algo_00_init(void)
{
	...
	ispu_fir_init_f32_opt(&fir_x, FIR_TAPS, coeffs, state_x);
	ispu_fir_init_f32_opt(&fir_y, FIR_TAPS, coeffs, state_y);
	ispu_fir_init_f32_opt(&fir_z, FIR_TAPS, coeffs, state_z);
}
...
void __attribute__ ((signal)) algo_00(void)
{
	if (full) {
		...
		ispu_fir_f32_opt(&fir_x, (float *)acc_x[proc], out_x[proc], BLOCK_SIZE);
		ispu_fir_f32_opt(&fir_y, (float *)acc_y[proc], out_y[proc], BLOCK_SIZE);
		ispu_fir_f32_opt(&fir_z, (float *)acc_z[proc], out_z[proc], BLOCK_SIZE);
	}
