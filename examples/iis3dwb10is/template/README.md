...
* ***src***:
  * ***main.c***: main source file template. This is where the user code must be implemented:
    ...
    * The user should fill in `algo_00_init` and `algo_00` functions respectively with the initialization and running code of the algorithm.
      ...
      Note that `algo_00_init` is executed only when the first data sample is generated by the sensor (after boot or after disabling and re-enabling the algorithm), while `algo_00` is executed every time the ISPU is woken up by the generation of a new data sample, in accordance to the ISPU rate configured.
    * Analogous functions (for example, `algo_01_init` and `algo_01`) can be added in order to implement additional algorithms (up to 16).
