...
* Implement the code to read the sensor data needed by the neural network and store it in global variables for later usage. The `data_valid_interrupt` routine is executed every time a new data sample is generated by the sensor.
...
As always, *ispu/conf.txt* and *ispu/meta.txt* should also be modified as required. For more information on these files, please refer to the regular (not specific for ST Edge AI) ISPU template.
