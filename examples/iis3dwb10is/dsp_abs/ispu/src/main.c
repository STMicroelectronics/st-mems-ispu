...
void __attribute__ ((signal)) algo_00(void)
{
	if (full) {
		...
		ispu_abs_f32((float *)acc_x[proc], out_x[proc], BLOCK_SIZE);
		ispu_abs_f32((float *)acc_y[proc], out_y[proc], BLOCK_SIZE);
		ispu_abs_f32((float *)acc_z[proc], out_z[proc], BLOCK_SIZE);
	}
