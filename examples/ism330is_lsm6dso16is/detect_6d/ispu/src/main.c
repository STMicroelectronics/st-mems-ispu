...
void __attribute__ ((signal)) algo_00_init(void)
{
	...
	struct detect_6d_conf d6d_conf = {
		.d4 = false,
		.ths = D6D_THS,
		.dur = D6D_DUR,
		.filt = { // IIR2 butter LP 1 Hz
			.b = { 0.012426292572095f, 0.024852585144190f, 0.012426292572095f },
			.a = { 1.000000000000000f, -1.660834370919655f, 0.710539541208035f },
			.fast_set = true
		}
	};

	detect_6d_init(&d6d, &d6d_conf);
}

void __attribute__ ((signal)) algo_00(void)
{
	struct detect_6d_in in;
