...
The notebook uses Keras and TensorFlow Lite to create, train, and quantize the model, but other frameworks could be used instead (any framework where the model can be exported or converted to ONNX, such as PyTorch, QKeras for quantization, etc.).
...
By running the following command the original (.keras) model can be analyzed:

```powershell
stedgeai analyze --target ispu --device imu_22 --model output_ipynb/cnn_8x8x8.keras --no-workspace
```
...
------------------------------------------------------------------------------------------------
model file         :   /st/st-mems-ispu/tutorials/st_edge_ai_keras/output_ipynb/cnn_8x8x8.keras
type               :   keras
c_name             :   network
...
```powershell
stedgeai analyze --target ispu --device imu_22 --model output_ipynb/qcnn_8x8x8.tflite --no-workspace
```
...
Another useful option is the ability to perform the validation on target to check the correctness of the converted model on the final target and obtain a measurement of its execution time. In order to do that, the Nucleo board must be flashed with the [dedicated firmware](../../host_firmware/nucleo_ispu_stedgeai_validate"). Then, it is possible to perform the validation by connecting the Nucleo board to a USB port of the PC, and running the `validate` command specifying the model (.keras) and the validation data (.npz) as arguments:

```powershell
stedgeai validate --target ispu --device imu_22 --mode target --model output_ipynb/cnn_8x8x8.keras --valinput output_ipynb/har_testset.npz --no-workspace
```
...
```
  ...
  ---------------------------------------------------------------
  nb sample(s)   :   527
  duration       :   67.953 ms by sample (67.782/68.213/0.102)
  macc           :   12960
  cycles/MACC    :   26.22
  ---------------------------------------------------------------
```

The most important piece of information here is given by the `duration` field, which reports that the model inference is taking ~68 ms for its execution; this is an important parameter to know for running the model in real time.
...
```
   ...
   ---------------------------------------------------------------------------------------------------------
   0       2      Conv2D (0x103) - Conv2D                        19.059   28.05%    28.05%     ai_node_0
   1       3      NL (0x107) - Nonlinearity (relu)                5.582    8.21%    36.26%     ai_node_1
   2       4      Pool (0x10b) - Pool_Max                         9.101   13.39%    49.65%     ai_node_2
   3       6      Conv2D (0x103) - Conv2D                        14.724   21.67%    71.32%     ai_node_3
   4       7      NL (0x107) - Nonlinearity (relu)                2.805    4.13%    75.45%     ai_node_4
   5       8      Pool (0x10b) - Pool_Max                         4.552    6.70%    82.15%     ai_node_5
   6       10     Conv2D (0x103) - Conv2D                         7.267   10.69%    92.84%     ai_node_6
   7       11     NL (0x107) - Nonlinearity (relu)                1.419    2.09%    94.93%     ai_node_7
   8       12     Pool (0x10b) - Pool_Max                         2.107    3.10%    98.03%     ai_node_8
   9       14     Dense (0x104) - Dense                           0.391    0.58%    98.61%     ai_node_9
   10      16     Softmax (0x10c) - Nonlinearity (softmax)        0.865    1.27%    99.88%     ai_node_10
   ---------------------------------------------------------------------------------------------------------
   n/a     n/a    Inter-nodal                                     0.082    0.12%   100.00%     n/a
   ---------------------------------------------------------------------------------------------------------
   total                                                         67.953
   ---------------------------------------------------------------------------------------------------------
```
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TARGET c-model #1   98.86%    0.072328001   0.012726214   0.146874368   0.000000    0.072345   16.793270    0.972113   0.989493   'output', 527 x f32(1x4), m_id=[16]
original model #1   98.86%    0.072328001   0.012726217   0.146874368   0.000000    0.072345   16.793270    0.972113   0.989493   'output', 527 x f32(1x4), m_id=[16]
X-cross #1          100.00%   0.000000078   0.000000022   0.000000158   -0.000000   0.000000   136.048231   1.000000   1.000000   'output', 527 x f32(1x4), m_id=[16]
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
```
...
By running the following command, the Keras (.keras) model can be converted:

```powershell
stedgeai generate --target ispu --device imu_22 --model output_ipynb/cnn_8x8x8.keras --output ispu_integration --no-workspace --no-report
```
...
Next, the variables to implement the application logic are needed. In this case, the number of samples currently stored inside the input buffer.
...
-358.436005	172.020004	958.432007	0.000000	0.000000	0.000000	0.000000	(0) stationary
-384.056000	298.656006	750.299988	0.000000	0.000000	0.000000	0.000000	(0) stationary
-294.752014	307.928009	556.075989	0.000000	0.000000	0.000000	0.000000	(0) stationary
-326.227997	228.139999	343.795990	0.000000	0.000000	0.000000	0.000000	(0) stationary
-445.787994	177.143997	283.283997	0.000000	0.000000	0.000000	0.000000	(0) stationary
-46.360001	-18.788000	161.772003	0.000000	0.000000	0.000000	0.000000	(0) stationary
-55.875999	118.828003	221.063995	0.000000	0.000000	0.000000	0.000000	(0) stationary
-989.419983	805.687988	672.219971	0.000000	0.000000	0.000000	0.000000	(0) stationary
...
-834.968018	805.687988	1391.776001	0.000105	0.986666	0.012337	0.000892	(1) walking
-491.660004	360.388000	704.671997	0.000105	0.986666	0.012337	0.000892	(1) walking
-547.536011	101.015999	233.264008	0.000105	0.986666	0.012337	0.000892	(1) walking
-407.235992	-57.096001	-0.976000	0.000105	0.986666	0.012337	0.000892	(1) walking
-107.848000	-183.000000	-287.432007	0.000105	0.986666	0.012337	0.000892	(1) walking
-56.852001	134.688004	-22.204000	0.000105	0.986666	0.012337	0.000892	(1) walking
-443.592010	518.987976	689.788025	0.000105	0.986666	0.012337	0.000892	(1) walking
-583.892029	618.539978	1338.583984	0.000105	0.986666	0.012337	0.000892	(1) walking
